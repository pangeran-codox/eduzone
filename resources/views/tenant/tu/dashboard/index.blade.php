...
<div class="t-card p-6">
    ...
    <div class="grid sm:grid-cols-2 gap-3">
        ...
        @if (Route::has('tu.absensi.index'))
        <a href="{{ route('tu.absensi.index') }}" class="flex items-center gap-3 p-4 rounded-xl border" style="border-color: var(--t-border);">
            <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style="background: var(--t-green-bg);">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="var(--t-green-tx)" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/>
                </svg>
            </div>
                        <div>
                <p class="text-sm font-semibold" style="color: var(--t-text);">Dashboard Absensi</p>
                <p class="text-xs" style="color: var(--t-muted);">Monitor kehadiran real-time hari ini</p>
            </div>
        </a>
        @endif

        @if (Route::has('tu.absensi.manual.index'))
        <a href="{{ route('tu.absensi.manual.index') }}" class="flex items-center gap-3 p-4 rounded-xl border" style="border-color: var(--t-border);">
            <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style="background: rgba(201,162,39,0.15);">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="#8A6D1B" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/>
                </svg>
            </div>
            <div>
                <p class="text-sm font-semibold" style="color: var(--t-text);">Input Manual Absensi</p>
                <p class="text-xs" style="color: var(--t-muted);">Catat Izin/Sakit/Alpa siswa & guru</p>
            </div>
        </a>
        @endif

        @if (Route::has('absensi.rekap.index'))
        <a href="{{ route('absensi.rekap.index') }}" class="flex items-center gap-3 p-4 rounded-xl border" style="border-color: var(--t-border);">
            <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style="background: var(--t-amber-bg);">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="var(--t-amber-tx)" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/>
                </svg>
            </div>
            <div>
                <p class="text-sm font-semibold" style="color: var(--t-text);">Rekap Absensi</p>
                <p class="text-xs" style="color: var(--t-muted);">Ringkasan kehadiran per kelas</p>
            </div>
        </a>
        @endif
